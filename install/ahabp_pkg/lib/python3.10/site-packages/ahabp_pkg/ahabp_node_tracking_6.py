...
from px4_msgs.msg import ( # These are compatible .msg files. Check 'px4_msgs/msg'
                        OffboardControlMode, VehicleCommand, VehicleAttitudeSetpoint, TrajectorySetpoint, # RPi4 --> PixRacer
                        VehicleStatus, SensorGps, VehicleAttitude, VehicleControlMode, VehicleLocalPosition, VehicleGlobalPosition # PixRacer --> RPi4
                        )
...
## NOT WORKING
        # self.vehicle_local_position_subscriber = self.create_subscription(VehicleLocalPosition, '/fmu/out/vehicle_local_position', self.vehicle_local_position_callback, qos_profile_sub)
...
#### Subscriber functions ####
    def vehicle_gps_position_callback(self, msg): # msg is the entire message of the px4_msg topic
        print('Here at')
        logger.info(f'Received vehicle_gps_position_callback: {msg}') # Debug print in case want to see entire message
        msg_parsed = (f'timestamp: {msg.timestamp} lat: {msg.lat} lon: {msg.lon} alt: {msg.alt} eph: {msg.eph} epv: {msg.epv} heading: {msg.heading} heading_accuracy: {msg.heading_accuracy} satellites_used: {msg.satellites_used} ') # Parsed message. Gotta look at available parameters by running 'ros2 topic echo (/topic)'
        self.vehicle_gps_position = msg_parsed # Save parsed message into self variable which can be used in timer callback function

    def vehicle_attitude_callback(self, msg): # WORKS
    ...
    def vehicle_status_callback(self, msg):
        logger.info(f'Received vehicle_status_callback: {msg}')
    ...
    def vehicle_local_position_callback(self, msg):
        logger.info(f'Received vehicle_local_position_callback: {msg}')
        msg_parsed = (f'timestamp: {msg.timestamp} ref_lat: {msg.ref_lat} ref_lon: {msg.ref_lon} x: {msg.x} y: {msg.y} z: {msg.z} heading: {msg.heading} ref_alt: {msg.ref_alt} ')
        self.vehicle_local_position = msg_parsed

    def vehicle_global_position_callback(self, msg): # WORKS
...
## Subscriber message prints
        # print('## self.vehicle_gps_position: ') # NO WORK
        # logger.info(self.vehicle_gps_position)

        print('## self.vehicle_attitude: ')
        logger.info(self.vehicle_attitude)

        print('## self.vehicle_status: ')
        logger.info(self.vehicle_status)

        # print('## self.vehicle_local_position: ') # NO WORK
        # logger.info(self.vehicle_local_position)

        print('## self.vehicle_global_position: ')
        logger.info(self.vehicle_global_position)
